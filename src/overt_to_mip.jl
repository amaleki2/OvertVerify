...
operations are turned into mix integer programs following mixed integer program
representation introduced in https://arxiv.org/abs/1711.07356
"""
...
function eq_2_mip(expr::Expr, overt_mip_model::OvertMIP)
    #println(expr)
    Overt.fix_negation!(expr)
    if Overt.is_max_expr(expr)
...
function affine_to_mip(expr::Expr, overt_mip_model::OvertMIP)
    ...
    var_list, coeff_list, scalar = Overt.get_linear_coeffs(expr.args[3])
...
function max_alone_2_mip(expr::Expr, outvar::JuMP.VariableRef, overt_mip_model::OvertMIP)
    var, coef, scalar = Overt.get_linear_coeffs(expr.args[3])
...
function max_min_2_mip(expr::Expr, outvar::JuMP.VariableRef, overt_mip_model::OvertMIP)
    ...
    var1, coef1, scalar1 = Overt.get_linear_coeffs(min_arg.args[2])
    ...
    var2, coef2, scalar2 = Overt.get_linear_coeffs(min_arg.args[3])
